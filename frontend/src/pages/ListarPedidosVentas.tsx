...
export default function ListarPedidosVentas() {
  ...
  const pedidosFiltrados = React.useMemo(() => {
    ...
    resultado.sort((a, b) => {
      switch (ordenamiento) {
        case 'fecha_desc': return new Date(b.fecha_pedido).getTime() - new Date(a.fecha_pedido).getTime();
        case 'fecha_asc': return new Date(a.fecha_pedido).getTime() - new Date(b.fecha_pedido).getTime();
        case 'numero_desc': return b.numero_pedido.localeCompare(a.numero_pedido);
        case 'numero_asc': return a.numero_pedido.localeCompare(b.numero_pedido);
        default: return 0;
      }
    });
  ...
  if (loading) return <div className="list-container"><LoadingSpinner message="Cargando pedidos de ventas..." size="large" /></div>;

  if (error) {
    return (
      <div className="list-container">
        <AlertMessage type="error" message={error} onClose={() => setError(null)} />
        <EmptyState icon="❌" title="Error al cargar pedidos" description={error} actionLabel="🔄 Reintentar" onAction={cargarPedidos} />
      </div>
    );
  }
  ...
  return (
    <div className="list-container">
      <PageHeader 
        ...
        icon="📥"
        actions={<Button onClick={cargarPedidos} icon="🔄" variant="secondary">Actualizar</Button>}
      />

      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '16px', marginBottom: '24px' }}>
        <StatsCard title="Pendientes" value={contarPorEstado('PENDIENTE')} icon="⏳" color="yellow" subtitle="Por procesar" />
        <StatsCard title="Procesados" value={contarPorEstado('PROCESADO')} icon="✅" color="green" subtitle="Con OT asignada" />
      </div>

      <div style={{ background: 'white', padding: '20px', borderRadius: '12px', marginBottom: '20px', border: '2px solid #e2e8f0' }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: '16px', marginBottom: '16px' }}>
          <SearchBar value={busqueda} onChange={setBusqueda} placeholder="N° pedido, cliente, dirección..." />
          <SelectField label="" name="estado" value={filtroEstado} onChange={setFiltroEstado} options={[
            ...
            { value: 'PROCESADO', label: `✅ Procesados (${contarPorEstado('PROCESADO')})` }
          ]} />
      ...
      {pedidosFiltrados.length === 0 ? (
        <EmptyState icon="📦" title="No hay pedidos" description="No se encontraron pedidos con los filtros aplicados" actionLabel="🗑️ Limpiar filtros" onAction={limpiarFiltros} />
      ) : (
        <>
          <p style={{ color: '#64748b', marginBottom: '16px' }}>📈 Mostrando {pedidosFiltrados.length} de {pedidos.length} pedidos</p>
          
          <div style={{ overflowX: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', backgroundColor: 'white', borderRadius: '12px', overflow: 'hidden' }}>
              <thead>
                <tr style={{ backgroundColor: '#f8fafc', borderBottom: '2px solid #e2e8f0' }}>
                  <th style={{ padding: '16px', textAlign: 'left', fontWeight: '700' }}>N° Pedido</th>
                  ...
                  <th style={{ padding: '16px', textAlign: 'left', fontWeight: '700' }}>Recepción</th>
                </tr>
              </thead>
              <tbody>
                {pedidosFiltrados.map((pedido) => (
                  <tr key={pedido.id} style={{ borderBottom: '1px solid #f1f5f9' }}
                    ...
                    <td style={{ padding: '16px', fontWeight: '600', color: '#667eea' }}>{pedido.numero_pedido}</td>
                    <td style={{ padding: '16px', color: '#334155', fontWeight: '500' }}>{pedido.cliente}</td>
                    <td style={{ padding: '16px', color: '#334155' }}>{new Date(pedido.fecha_pedido).toLocaleDateString('es-CL')}</td>
          ...
          <InfoBox title="💡 Información" variant="tip">
            <strong>Pedidos Pendientes:</strong> Esperan ser procesados en una OT de Picking<br />
            <strong>Dirección de Despacho:</strong> Viene automáticamente del pedido original<br />
            <strong>Procesados:</strong> Ya tienen una OT asociada<br />
