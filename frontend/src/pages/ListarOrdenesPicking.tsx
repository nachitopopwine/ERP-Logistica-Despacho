...
const ListarOrdenesPicking: React.FC = () => {
  ...
  const ordenesFiltradas = React.useMemo(() => {
    ...
    resultado.sort((a, b) => {
      switch (ordenamiento) {
        ...
        case "id_desc": return b.id_ot - a.id_ot;
        case "id_asc": return a.id_ot - b.id_ot;
        default: return 0;
      }
    });
  ...
  const getEstadoVariant = (estado: string): 'pendiente' | 'proceso' | 'completado' | 'cancelado' => {
  ...
  if (loading) return <div className="list-container"><LoadingSpinner message="Cargando órdenes..." size="large" /></div>;
  ...
  return (
    <div className="list-container">
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '16px', marginBottom: '24px' }}>
        ...
        <StatsCard title="En Proceso" value={contarPorEstado('En proceso')} icon="🔄" color="blue" subtitle="Activas" />
        <StatsCard title="Completadas" value={contarPorEstado('Completado')} icon="✅" color="green" subtitle="Finalizadas" />
      </div>

      <div style={{ background: 'white', padding: '20px', borderRadius: '12px', marginBottom: '20px', border: '2px solid #e2e8f0' }}>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(250px, 1fr))", gap: "16px", marginBottom: '16px' }}>
          <SearchBar value={busqueda} onChange={setBusqueda} placeholder="ID, empleado, observaciones..." />
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', backgroundColor: 'white', borderRadius: '12px', overflow: 'hidden' }}>
              <thead>
                <tr style={{ backgroundColor: '#f8fafc', borderBottom: '2px solid #e2e8f0' }}>
                  ...
                  <th style={{ padding: '16px', textAlign: 'left', fontWeight: '700' }}>Empleado</th>
              ...
              <tbody>
                {ordenesFiltradas.map((orden) => (
                  <tr key={orden.id_ot} style={{ borderBottom: '1px solid #f1f5f9' }}
                    onMouseEnter={(e) => e.currentTarget.style.backgroundColor = '#f8fafc'}
                    onMouseLeave={(e) => e.currentTarget.style.backgroundColor = 'white'}>
                    <td style={{ padding: '16px', fontWeight: '600', color: '#667eea' }}>OT #{orden.id_ot}</td>
                    <td style={{ padding: '16px', color: '#334155' }}>{new Date(orden.fecha).toLocaleDateString('es-CL')}</td>
                    <td style={{ padding: '16px', color: '#334155' }}>{orden.nombre_empleado || `ID: ${orden.id_empleado}`}</td>
                    <td style={{ padding: '16px', textAlign: 'center' }}>
                      <Badge variant={getEstadoVariant(orden.estado)}>{orden.estado}</Badge>
                    </td>
                    <td style={{ padding: '16px', color: '#4a5568', maxWidth: '300px' }}>
                      {orden.observaciones || <span style={{ color: '#a0aec0', fontStyle: 'italic' }}>Sin observaciones</span>}
                    </td>
                  </tr>
                ))}
              </tbody>
