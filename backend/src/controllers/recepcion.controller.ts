 ...
 */
export const getAllRecepciones = async (
  _req: Request,
  res: Response
): Promise<void> => {
  try {
    const result = await pool.query(`
      SELECT 
        ...
        r.fecha,
        r.observaciones
      FROM "Logistica".log_recepcion r
      LEFT JOIN public.proveedor prov ON r.id_proveedor = prov.id_proveedor
      LEFT JOIN public.producto prod ON r.id_producto = prod.id_producto
 ...
 */
export const getRecepcionById = async (
  req: Request,
  res: Response
): Promise<void> => {
  try {
...
export const createRecepcion = async (
  ...
  try {
    const { id_proveedor, id_producto, cantidad, fecha, observaciones } =
      req.body;
    ...
    if (!id_proveedor || !id_producto || !cantidad || !fecha) {
      res.status(400).json({
        success: false,
        message:
          "Faltan campos requeridos: id_proveedor, id_producto, cantidad y fecha son obligatorios",
      });
      return;
    }
    ...
    // Insertar la recepción en Logistica
    const result = await pool.query(
      `
      INSERT INTO "Logistica".log_recepcion (id_proveedor, id_producto, cantidad, fecha, observaciones)
      VALUES ($1, $2, $3, $4, $5)
      RETURNING *
    `,
