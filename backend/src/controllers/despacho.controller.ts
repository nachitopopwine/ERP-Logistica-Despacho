  ...
  try {
    ...
    if (!id_ot || !fecha || !transportista) {
      res.status(400).json({
        success: false,
        message:
          "Faltan campos requeridos: id_ot, fecha y transportista son obligatorios",
      });
      return;
    }

    // ⚠️ VALIDACIÓN: Verificar que la OT exista en el esquema Logistica
    ...
    // Crear la guía en el esquema Logistica
    const result = await pool.query(
      `
      INSERT INTO "Logistica".log_guia_despacho (id_ot, fecha, transportista, direccion_entrega)
      VALUES ($1, $2, $3, $4)
      RETURNING *
    `,
